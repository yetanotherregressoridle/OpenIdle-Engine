...
import { GlobalEffectConfig, ResourceState, ActionConfig, ActionState, Prerequisite, ResourceID, ActionID, EncounterState, ItemState, ItemID, AllyCombatState, EnemyCombatState, PartyUnitID, MainCharacterPartyUnitID, UnitEffect, UnitState, PartyUnitState, SkillID, EnemyUnitID, UnitEffectOptions, ThemeID, Tab, DefaultTab, ProgressionID } from "./types";
...
interface State {
    ...
    tabs: Tab[];
    activeTab: Tab;
    progression: Record<ProgressionID, boolean>;
    metaProgression: Record<ProgressionID, boolean>;
}
...
const initialState: State = {
    ...
    logs: [],
    theme: "system",
    tabs: ['actions', 'encounter', 'inventory', 'config'],
...
interface StateApi {
    ...
    setProgression: (progressionID: ProgressionID) => void;
    setMetaProgression: (progressionID: ProgressionID) => void;
}
...
export function createStateStore(): StateStore {
    ...
    const stateApi: StateApi = {
        ...
        setActiveTab: (tab: Tab) => {
            setStateStore("activeTab", tab);
        },
        setProgression: (progressionID: ProgressionID) => {
            setStateStore("progression", progressionID, true);
        },
        setMetaProgression: (progressionID: ProgressionID) => {
            setStateStore("metaProgression", progressionID, true);
        },
    }
    return [
        stateStore,
...
function checkVisibility(state: State, id: ResourceID | ActionID): boolean {
    if (id in RESOURCE_CONFIG_MAP) {
        ...
        if (id in state.resources) {
            visible = state.resources[id].visible && !RESOURCE_CONFIG_MAP[id].hidden && state.resources[id].max !== 0;
        }
    ...
    if (!prerequisites || prerequisites.length === 0) {
        ...
        enabled = prerequisites.every(prereq => {
            if (prereq.resourceID) {
                ...
                if (prereq.minMax !== undefined && res.max !== undefined && res.max < prereq.minMax) return false;
                if (prereq.maxMax !== undefined && res.max !== undefined && res.max > prereq.maxMax) return false;

                if (prereq.valueLessThanMax && res.max !== undefined && res.value >= res.max) return false;
                if (prereq.valueGreaterThanZero && res.value <= 0) return false;
                if (prereq.valueEqualToMax && res.max !== undefined && res.value !== res.max) return false;
            }
            if (prereq.actionID) {
            ...
            if (prereq.progressionID) {
                const prog = state.progression[prereq.progressionID];
                if (!prog) return false;
            }
            if (prereq.metaProgressionID) {
                const prog = state.metaProgression[prereq.metaProgressionID];
                if (!prog) return false;
            }
    ...
    switch (effect.type) {
        case "increase_value":
            ...
            if (effect.resourceID) {
                ...
                setStateStore("resources", effect.resourceID, "value", (current) => {
                    let value = current + (effect.amount || 0);
                    let max = state.resources[effect.resourceID || ""]?.max || 0;
                    if (max !== -1) {
        ...
        case "increase_max":
            ...
            if (effect.resourceID) {
                ...
                if (state.resources[effect.resourceID || ""]?.max === -1) {
                    console.log("Cannot increase max for resource " + effect.resourceID + " because it has a max of -1.");
                    return;
                }
        ...
        case "start_encounter":
            ...
            stateApi.setActiveTab("encounter");
            break;
        case "enable_progression":
            if (!effect.progressionID) {
                console.log('no progression id found');
                return;
            }
            stateApi.setProgression(effect.progressionID);
            break;
        case "enable_meta_progression":
            if (!effect.metaProgressionID) {
                console.log('no meta progression id found');
                return;
            }
            stateApi.setMetaProgression(effect.metaProgressionID);
            break;
        case "loop":
            console.log("looping");
            for (const res of Object.keys(state.resources)) {
                setStateStore("resources", res, "value", 0);
            }
            for (const action of Object.keys(state.actions)) {
                setStateStore("actions", action, "executions", 0);
            }
            setStateStore("resources", "regressions", "value", current => current + 1);
            setStateStore("progression", {});
            break;
    }
}
...
function clampResource(arg0: number, arg1: number, max: number) {
